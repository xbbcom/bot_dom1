...
sub chek_build_1 {
		...
		if($res[0] eq 200){
			...
			my $elem1 = $tree -> look_down( _tag => 'div', class => 'boxes buildingList');
			if(defined $elem1){
				my $elem2 = $elem1 -> look_down(_tag => 'div', class => 'buildDuration');
					if(defined $elem2){
						my $elem3 = $elem2 -> look_down(_tag => 'span', class => 'timer');
						my $t = $elem3 -> as_text, "\n";
						my @time = split(':',$t);
						my $sec = $time[0]*3600;
						$sec += $time[1]*60;
						$sec += $time[2];
						print LOG scalar(localtime), ' chek_build_1() Все строители заняты. Сон на ', $sec, ' сек.', "\n";
					}
			}
		}
				} else {
