...
sub ua_send_two($$){
	...
	#print STR scalar(localtime), ' ua_sen_two() $resp', " $resp \n";
	if ($resp -> is_success){
		$ras = 0;
		#my $resp_all = $resp->as_string;
		#print LOG 'ua_send_two($$)', "\n $resp \n";
		my $resp_body = $resp -> content;
		#print CONT 'ua_send_two($$)', "\n $resp_body \n";
		return(200, $resp_body);
		} else {
			print LOG scalar(localtime), ' ua_send_two($$) Error: ' . $resp -> status_line . "\n" . $resp -> is_error . "\n";
			#$ras++;
			sleep 30;
			#if ($ras <= 10){
				ua_send_two($ua_metod, $ua_t_url);
...
sub ua_send_three($$$){
	...
	if ($resp -> is_success) {
		...
		return(200, $resp_body);
			...
			print LOG scalar(localtime), ' ua_send_three($$$) Error: ' . $resp -> status_line . "\n" . $resp -> is_error . "\n";
			#$ras++;
			sleep 30;
			#if ($ras <= 10){
				ua_send_three($ua_metod, $ua_t_url, $ua_par);
...
#@s = ua('get', 'http://ts1.travian.ru/dorf1.php');
#print STR 'ua.pl ',"\n @s \n";